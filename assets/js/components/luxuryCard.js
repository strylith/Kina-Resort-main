...
window.handleBookNow = function(packageName) {
  // For "Standard Room", open calendar modal to select dates first
  if (packageName === 'Standard Room' || packageName.includes('Standard Room')) {
    // Open calendar modal for room date selection
    if (window.openCalendarModal) {
      window.openCalendarModal('Standard Room', 4, 'rooms');
    } else {
  ...
  card.innerHTML = `
    ...
    <div class="category-content">
      ...
      <div class="category-info-expanded">
        ...
        <button class="category-book-btn" onclick="handleBookNow('${title}')">
          Book Now
